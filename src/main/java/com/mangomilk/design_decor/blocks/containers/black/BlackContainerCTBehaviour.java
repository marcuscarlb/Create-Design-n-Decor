...
public class BlackContainerCTBehaviour extends ConnectedTextureBehaviour.Base {

	@Override
	public CTSpriteShiftEntry getShift(BlockState state, Direction direction, @Nullable TextureAtlasSprite sprite) {
		Axis containerBlockAxis = BlackContainerBlock.getContainerBlockAxis(state);
		boolean small = !BlackContainerBlock.isLarge(state);
		if (containerBlockAxis == null)
			return null;

		if (direction.getAxis() == containerBlockAxis)
			return CDDSpriteShifts.BLACK_CONTAINER_FRONT.get(small);
		if (direction == Direction.UP)
			return CDDSpriteShifts.BLACK_CONTAINER_TOP.get(small);
		if (direction == Direction.DOWN)
			return CDDSpriteShifts.BLACK_CONTAINER_BOTTOM.get(small);

		return CDDSpriteShifts.BLACK_CONTAINER_SIDE.get(small);
	}
