...
public class BrownContainerCTBehaviour extends ConnectedTextureBehaviour.Base {

	@Override
	public CTSpriteShiftEntry getShift(BlockState state, Direction direction, @Nullable TextureAtlasSprite sprite) {
		Axis containerBlockAxis = BrownContainerBlock.getContainerBlockAxis(state);
		boolean small = !BrownContainerBlock.isLarge(state);
		if (containerBlockAxis == null)
			return null;

		if (direction.getAxis() == containerBlockAxis)
			return CDDSpriteShifts.BROWN_CONTAINER_FRONT.get(small);
		if (direction == Direction.UP)
			return CDDSpriteShifts.BROWN_CONTAINER_TOP.get(small);
		if (direction == Direction.DOWN)
			return CDDSpriteShifts.BROWN_CONTAINER_BOTTOM.get(small);

		return CDDSpriteShifts.BROWN_CONTAINER_SIDE.get(small);
	}
