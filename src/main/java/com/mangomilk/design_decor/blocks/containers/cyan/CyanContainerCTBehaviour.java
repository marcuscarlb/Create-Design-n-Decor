...
public class CyanContainerCTBehaviour extends ConnectedTextureBehaviour.Base {

	@Override
	public CTSpriteShiftEntry getShift(BlockState state, Direction direction, @Nullable TextureAtlasSprite sprite) {
		Axis containerBlockAxis = CyanContainerBlock.getContainerBlockAxis(state);
		boolean small = !CyanContainerBlock.isLarge(state);
		if (containerBlockAxis == null)
			return null;

		if (direction.getAxis() == containerBlockAxis)
			return CDDSpriteShifts.CYAN_CONTAINER_FRONT.get(small);
		if (direction == Direction.UP)
			return CDDSpriteShifts.CYAN_CONTAINER_TOP.get(small);
		if (direction == Direction.DOWN)
			return CDDSpriteShifts.CYAN_CONTAINER_BOTTOM.get(small);

		return CDDSpriteShifts.CYAN_CONTAINER_SIDE.get(small);
	}
