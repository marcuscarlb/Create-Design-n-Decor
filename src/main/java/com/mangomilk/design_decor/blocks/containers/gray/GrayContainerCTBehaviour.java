...
public class GrayContainerCTBehaviour extends ConnectedTextureBehaviour.Base {

	@Override
	public CTSpriteShiftEntry getShift(BlockState state, Direction direction, @Nullable TextureAtlasSprite sprite) {
		Axis containerBlockAxis = GrayContainerBlock.getContainerBlockAxis(state);
		boolean small = !GrayContainerBlock.isLarge(state);
		if (containerBlockAxis == null)
			return null;

		if (direction.getAxis() == containerBlockAxis)
			return CDDSpriteShifts.GRAY_CONTAINER_FRONT.get(small);
		if (direction == Direction.UP)
			return CDDSpriteShifts.GRAY_CONTAINER_TOP.get(small);
		if (direction == Direction.DOWN)
			return CDDSpriteShifts.GRAY_CONTAINER_BOTTOM.get(small);

		return CDDSpriteShifts.GRAY_CONTAINER_SIDE.get(small);
	}
