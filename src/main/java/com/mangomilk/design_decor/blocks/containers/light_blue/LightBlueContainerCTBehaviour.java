...
public class LightBlueContainerCTBehaviour extends ConnectedTextureBehaviour.Base {

	@Override
	public CTSpriteShiftEntry getShift(BlockState state, Direction direction, @Nullable TextureAtlasSprite sprite) {
		Axis containerBlockAxis = LightBlueContainerBlock.getContainerBlockAxis(state);
		boolean small = !LightBlueContainerBlock.isLarge(state);
		if (containerBlockAxis == null)
			return null;

		if (direction.getAxis() == containerBlockAxis)
			return CDDSpriteShifts.LIGHT_BLUE_CONTAINER_FRONT.get(small);
		if (direction == Direction.UP)
			return CDDSpriteShifts.LIGHT_BLUE_CONTAINER_TOP.get(small);
		if (direction == Direction.DOWN)
			return CDDSpriteShifts.LIGHT_BLUE_CONTAINER_BOTTOM.get(small);

		return CDDSpriteShifts.LIGHT_BLUE_CONTAINER_SIDE.get(small);
	}
