...
public class LightBlueContainerItem extends BlockItem {

	public LightBlueContainerItem(Block p_i48527_1_, Properties p_i48527_2_) {
	...
	public void tryMultiPlace(BlockPlaceContext ctx) {
		...
		if (!LightBlueContainerBlock.isContainer(placedOnState))
			return;
		LightBlueContainerBlockEntity tankAt = ConnectivityHandler.partAt(CDDBlockEntities.BLUE_CONTAINER.get(), world, placedOnPos);
		if (tankAt == null)
			return;
		LightBlueContainerBlockEntity controllerBE = tankAt.getControllerBE();
		...
		int tanksToPlace = 0;
		Axis vaultBlockAxis = LightBlueContainerBlock.getContainerBlockAxis(placedOnState);
		...
		for (int xOffset = 0; xOffset < width; xOffset++) {
			for (int zOffset = 0; zOffset < width; zOffset++) {
				...
				if (LightBlueContainerBlock.isContainer(blockState))
					continue;
