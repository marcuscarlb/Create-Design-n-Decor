...
public class LightGrayContainerCTBehaviour extends ConnectedTextureBehaviour.Base {

	@Override
	public CTSpriteShiftEntry getShift(BlockState state, Direction direction, @Nullable TextureAtlasSprite sprite) {
		Axis containerBlockAxis = LightGrayContainerBlock.getContainerBlockAxis(state);
		boolean small = !LightGrayContainerBlock.isLarge(state);
		if (containerBlockAxis == null)
			return null;

		if (direction.getAxis() == containerBlockAxis)
			return CDDSpriteShifts.LIGHT_GRAY_CONTAINER_FRONT.get(small);
		if (direction == Direction.UP)
			return CDDSpriteShifts.LIGHT_GRAY_CONTAINER_TOP.get(small);
		if (direction == Direction.DOWN)
			return CDDSpriteShifts.LIGHT_GRAY_CONTAINER_BOTTOM.get(small);

		return CDDSpriteShifts.LIGHT_GRAY_CONTAINER_SIDE.get(small);
	}
