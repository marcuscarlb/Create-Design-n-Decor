...
public class MagentaContainerCTBehaviour extends ConnectedTextureBehaviour.Base {

	@Override
	public CTSpriteShiftEntry getShift(BlockState state, Direction direction, @Nullable TextureAtlasSprite sprite) {
		Axis containerBlockAxis = MagentaContainerBlock.getContainerBlockAxis(state);
		boolean small = !MagentaContainerBlock.isLarge(state);
		if (containerBlockAxis == null)
			return null;

		if (direction.getAxis() == containerBlockAxis)
			return CDDSpriteShifts.MAGENTA_CONTAINER_FRONT.get(small);
		if (direction == Direction.UP)
			return CDDSpriteShifts.MAGENTA_CONTAINER_TOP.get(small);
		if (direction == Direction.DOWN)
			return CDDSpriteShifts.MAGENTA_CONTAINER_BOTTOM.get(small);

		return CDDSpriteShifts.MAGENTA_CONTAINER_SIDE.get(small);
	}
