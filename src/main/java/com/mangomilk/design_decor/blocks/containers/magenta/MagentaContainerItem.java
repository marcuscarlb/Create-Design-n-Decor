...
public class MagentaContainerItem extends BlockItem {

	public MagentaContainerItem(Block p_i48527_1_, Properties p_i48527_2_) {
	...
	public void tryMultiPlace(BlockPlaceContext ctx) {
		...
		if (!MagentaContainerBlock.isContainer(placedOnState))
			return;
		MagentaContainerBlockEntity tankAt = ConnectivityHandler.partAt(CDDBlockEntities.BLUE_CONTAINER.get(), world, placedOnPos);
		if (tankAt == null)
			return;
		MagentaContainerBlockEntity controllerBE = tankAt.getControllerBE();
		...
		int tanksToPlace = 0;
		Axis vaultBlockAxis = MagentaContainerBlock.getContainerBlockAxis(placedOnState);
		...
		for (int xOffset = 0; xOffset < width; xOffset++) {
			for (int zOffset = 0; zOffset < width; zOffset++) {
				...
				if (MagentaContainerBlock.isContainer(blockState))
					continue;
