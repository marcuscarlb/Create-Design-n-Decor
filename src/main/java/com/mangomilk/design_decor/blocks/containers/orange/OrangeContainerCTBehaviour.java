...
public class OrangeContainerCTBehaviour extends ConnectedTextureBehaviour.Base {

	@Override
	public CTSpriteShiftEntry getShift(BlockState state, Direction direction, @Nullable TextureAtlasSprite sprite) {
		Axis containerBlockAxis = OrangeContainerBlock.getContainerBlockAxis(state);
		boolean small = !OrangeContainerBlock.isLarge(state);
		if (containerBlockAxis == null)
			return null;

		if (direction.getAxis() == containerBlockAxis)
			return CDDSpriteShifts.ORANGE_CONTAINER_FRONT.get(small);
		if (direction == Direction.UP)
			return CDDSpriteShifts.ORANGE_CONTAINER_TOP.get(small);
		if (direction == Direction.DOWN)
			return CDDSpriteShifts.ORANGE_CONTAINER_BOTTOM.get(small);

		return CDDSpriteShifts.ORANGE_CONTAINER_SIDE.get(small);
	}
