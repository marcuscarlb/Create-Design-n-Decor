...
public class PinkContainerItem extends BlockItem {

	public PinkContainerItem(Block p_i48527_1_, Properties p_i48527_2_) {
	...
	public void tryMultiPlace(BlockPlaceContext ctx) {
		...
		if (!PinkContainerBlock.isContainer(placedOnState))
			return;
		PinkContainerBlockEntity tankAt = ConnectivityHandler.partAt(CDDBlockEntities.BLUE_CONTAINER.get(), world, placedOnPos);
		if (tankAt == null)
			return;
		PinkContainerBlockEntity controllerBE = tankAt.getControllerBE();
		...
		int tanksToPlace = 0;
		Axis vaultBlockAxis = PinkContainerBlock.getContainerBlockAxis(placedOnState);
		...
		for (int xOffset = 0; xOffset < width; xOffset++) {
			for (int zOffset = 0; zOffset < width; zOffset++) {
				...
				if (PinkContainerBlock.isContainer(blockState))
					continue;
