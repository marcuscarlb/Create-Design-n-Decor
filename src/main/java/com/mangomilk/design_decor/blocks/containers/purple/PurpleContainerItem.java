...
public class PurpleContainerItem extends BlockItem {

	public PurpleContainerItem(Block p_i48527_1_, Properties p_i48527_2_) {
	...
	public void tryMultiPlace(BlockPlaceContext ctx) {
		...
		if (!PurpleContainerBlock.isContainer(placedOnState))
			return;
		PurpleContainerBlockEntity tankAt = ConnectivityHandler.partAt(CDDBlockEntities.BLUE_CONTAINER.get(), world, placedOnPos);
		if (tankAt == null)
			return;
		PurpleContainerBlockEntity controllerBE = tankAt.getControllerBE();
		...
		int tanksToPlace = 0;
		Axis vaultBlockAxis = PurpleContainerBlock.getContainerBlockAxis(placedOnState);
		...
		for (int xOffset = 0; xOffset < width; xOffset++) {
			for (int zOffset = 0; zOffset < width; zOffset++) {
				...
				if (PurpleContainerBlock.isContainer(blockState))
					continue;
