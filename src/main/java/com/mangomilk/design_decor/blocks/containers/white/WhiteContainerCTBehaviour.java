...
public class WhiteContainerCTBehaviour extends ConnectedTextureBehaviour.Base {

	@Override
	public CTSpriteShiftEntry getShift(BlockState state, Direction direction, @Nullable TextureAtlasSprite sprite) {
		Axis containerBlockAxis = WhiteContainerBlock.getContainerBlockAxis(state);
		boolean small = !WhiteContainerBlock.isLarge(state);
		if (containerBlockAxis == null)
			return null;

		if (direction.getAxis() == containerBlockAxis)
			return CDDSpriteShifts.WHITE_CONTAINER_FRONT.get(small);
		if (direction == Direction.UP)
			return CDDSpriteShifts.WHITE_CONTAINER_TOP.get(small);
		if (direction == Direction.DOWN)
			return CDDSpriteShifts.WHITE_CONTAINER_BOTTOM.get(small);

		return CDDSpriteShifts.WHITE_CONTAINER_SIDE.get(small);
	}
