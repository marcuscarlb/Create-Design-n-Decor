...
public class YellowContainerCTBehaviour extends ConnectedTextureBehaviour.Base {

	@Override
	public CTSpriteShiftEntry getShift(BlockState state, Direction direction, @Nullable TextureAtlasSprite sprite) {
		Axis containerBlockAxis = YellowContainerBlock.getContainerBlockAxis(state);
		boolean small = !YellowContainerBlock.isLarge(state);
		if (containerBlockAxis == null)
			return null;

		if (direction.getAxis() == containerBlockAxis)
			return CDDSpriteShifts.YELLOW_CONTAINER_FRONT.get(small);
		if (direction == Direction.UP)
			return CDDSpriteShifts.YELLOW_CONTAINER_TOP.get(small);
		if (direction == Direction.DOWN)
			return CDDSpriteShifts.YELLOW_CONTAINER_BOTTOM.get(small);

		return CDDSpriteShifts.YELLOW_CONTAINER_SIDE.get(small);
	}
