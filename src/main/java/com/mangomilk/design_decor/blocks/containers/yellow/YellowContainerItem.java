...
public class YellowContainerItem extends BlockItem {

	public YellowContainerItem(Block p_i48527_1_, Properties p_i48527_2_) {
	...
	public void tryMultiPlace(BlockPlaceContext ctx) {
		...
		if (!YellowContainerBlock.isContainer(placedOnState))
			return;
		YellowContainerBlockEntity tankAt = ConnectivityHandler.partAt(CDDBlockEntities.BLUE_CONTAINER.get(), world, placedOnPos);
		if (tankAt == null)
			return;
		YellowContainerBlockEntity controllerBE = tankAt.getControllerBE();
		...
		int tanksToPlace = 0;
		Axis vaultBlockAxis = YellowContainerBlock.getContainerBlockAxis(placedOnState);
		...
		for (int xOffset = 0; xOffset < width; xOffset++) {
			for (int zOffset = 0; zOffset < width; zOffset++) {
				...
				if (YellowContainerBlock.isContainer(blockState))
					continue;
